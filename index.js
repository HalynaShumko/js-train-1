...
function checkDataType(value) {
	...
	if (typeof value === 'number') {
		...
		return `Значення ${value} має тип "underfined"`;
	} else {
		return `error`;
	}
...
function getVariableType(variable) {
	...
	let m;
	m = typeof variable === 'number' ? "Це число" : 
	m = typeof variable === 'string' ? "Це рядок": 
	m = typeof variable === 'boolean' ? "Це булеве значення": "Це інший тип";
	return m;
...
function memoizedArea() {
	...
	return (side) => {
		if (side === prevSide) {
			return console.log(`Fetching from cache`), prevResult;
		}
		const result = `${side * side}`;
		prevSide = side;
		prevResult = result;
		return console.log(`Calculating result`), result;
	};
}
...
let cube = function (n) {return n = n * n * n;
  // Повертаємо n в кубі
};
